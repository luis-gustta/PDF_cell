...
def central_pulse(arr, d, theta, p, prob=1):
    n_x = len(arr[0, :, 0, 0])
    n_y = len(arr[:, 0, 0, 0])
    if d % 2 == 0:
        d += 1
    ...
    for i in index_arr:
        for j in index_arr:
            arr[int(n_y / 2.) + i, int(n_x / 2.) + j, int(theta), int(p)] = prob/(d**2)
    return arr
...
class Tissue(object):
            ...
            for x in range(0, self.n_x):
                for i in range(0, self.n_theta):
                    for j in range(0, self.n_pol):
                        ...
                        aux[y, x, i, j] = self.lattice[y, x, i, j] + .5*D*(self.lattice[y_plus, x, i, j] + self.lattice[y, x_plus, i, j]
        ...
        for y in range(0, self.n_y):
            for x in range(0, self.n_x):
                for i in range(0, self.n_theta):
                    for j in range(0, self.n_pol):
                        if j == 0:
                            aux[y, x, i, j] = self.real_lattice[y, x, i, j] + .5*D*(self.real_lattice[y, x, i, j + 1]
                                                                                - self.real_lattice[y, x, i, j])
                        if j == self.n_pol - 1:
                            aux[y, x, i, j] = self.real_lattice[y, x, i, j] + .5*D*(self.real_lattice[y, x, i, j - 1]
                                                                                - self.real_lattice[y, x, i, j])
                        if j > 0 and j < self.n_pol - 1:
                            aux[y, x, i, j] = self.real_lattice[y, x, i, j] + .5*D*(self.real_lattice[y, x, i, j + 1] + self.real_lattice[y, x, i, j - 1]
                                                                                - 2*self.real_lattice[y, x, i, j])
            ...
            for y in range(0, self.n_x):
                for i in range(0, self.n_theta):
                    ...
                    if self.real_lattice[y, x, i, 0] >= rho_n:
                        ...
                        self.real_lattice[y, x, i, int(self.n_pol/2)] = self.real_lattice[y, x, i, int(self.n_pol/2)] + rho_n
    ...
    def diffusion_theta(self):
        ...
        for i in range(0, self.n_theta):
            ...
            if i_plus > self.n_theta - 1:
                i_plus = 0
            aux[:, :, i, :] = self.real_lattice[:, :, i, :] + D*0.5*(self.real_lattice[:, :, i_min, :] +
                                                                     self.real_lattice[:, :, i_plus, :] - 2.*self.real_lattice[:, :, i, :])
            ...
            for j in range(0, self.n_pol):
                for x in range(0, self.n_x):
                    for y in range(0, self.n_y):
                        ...
                        if j == self.n_pol - 1:
                            aux[y, x, i, self.n_pol - 1] = self.real_lattice[y, x, i, self.n_pol - 1] - gamma*self.real_lattice[y, x, i, self.n_pol - 1]
                        if j != 0 and j != self.n_pol - 1:
                            aux[y, x, i, j] = self.real_lattice[y, x, i, j] + gamma*(self.real_lattice[y, x, i, j + 1]-self.real_lattice[y, x, i, j])
        self.real_lattice = np.copy(aux)
                ...
                for j in range(0, self.n_pol):
                    if self.real_lattice[:, :, i, j].any():
                        for x in range(0, self.n_x):
                            if self.real_lattice[:, x, i, j].any():
                                for y in range(0, self.n_y):
                                    if x > int(self.n_x / 2.) + int(self.max_x / 2.):
                                        ...
                                        self.real_lattice[y, x - int(self.max_x), i, j] = self.real_lattice[y, x - int(
                                            self.max_x), i, j] + aux
                                    if y > int(self.n_x / 2.) + int(self.max_y / 2.):
                                        aux = np.copy(self.real_lattice[y, x, i, j])
                                        self.real_lattice[y, x, i, j] = 0.
    ...
    def v_mean(self, theta, p, ori:list):
        sqrt2 = np.sqrt(2)
        self.val_x, self.val_y =  abs(int(1/2*(self.n_x-(sqrt2*self.n_x)))), abs(int(1/2*(self.n_y-(sqrt2*self.n_y))))
        v_c_x, v_c_y = 1, 1
        ...
        if self.max_val(theta, p)[1][0] >= self.real_max_y:
            ...
            self.v_mean_val = np.sqrt((self.max_val(theta, p)[1][1]+self.val_x-x_0)**2 + (self.max_val(theta, p)[1][0]-y_0)**2)/self.curr_time
        ...
        return self.v_mean_val, int(self.v_mean_val)
    ...
    def v_inst(self, theta, p):
        ...
        return self.v_inst_val, int(self.v_inst_val)
